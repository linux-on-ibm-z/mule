...
public class RestServiceWrapperFunctionalTestCase extends FunctionalTestCase {
  ...
  public void testErrorExpressionOnRegexFilterFail() throws Exception {
    Error error =
        muleContext.getClient().send("restServiceEndpoint", MuleMessage.builder().payload(TEST_REQUEST).build()).getLeft();
    assertEquals(RestServiceException.class, error.getException().getClass());
  }
  ...
  public void testRequiredParametersMissing() throws Exception {
    Error error = muleContext.getClient().send("restServiceEndpoint3", MuleMessage.builder().nullPayload().build()).getLeft();
    assertEquals(ComponentException.class, error.getException().getClass());
  }
