...
public class QueryMetadataTestCase extends MetadataExtensionFunctionalTestCase {
  ...
  private static final MetadataKey INVALID_METADATA_KEY = newKey("invalid").build();
  ...
  public void getEntityKeys() throws Exception {
    MetadataResult<Set<MetadataKey>> metadataKeysResult = metadataManager.getEntityKeys(QUERY_ID);
    assertThat(metadataKeysResult.isSuccess(), is(true));
    Set<MetadataKey> metadataKeys = metadataKeysResult.get();
  ...
  public void getDsqlQueryAutomaticGeneratedOutputMetadata() throws Exception {
    ...
    assertThat(generatedType, is(instanceOf(ObjectType.class)));

    Collection<ObjectFieldType> fields = ((ObjectType) generatedType).getFields();
    assertThat(fields, hasSize(1));
    ObjectFieldType field = fields.iterator().next();
